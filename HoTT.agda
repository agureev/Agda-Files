...
_,_ : {l1 l2 : Level} {A : Type l1} {B : Type l2} → A → B → A × B
a , b = pair a b
...
ap-comp :  {l1 l2 l3 : Level} {A : Type l1} {B : Type l2} {C : Type l3} {x y : A} (p : x ≡ y) (f : A → B) (g : B → C) → (fun-ap g (fun-ap f p)) ≡ (fun-ap (g ∘ f) p)
...
fun-ap-dep :  {l1 l2 : Level} {A : Type l1} {B : A → Type l2} (f : ∀ (x : A) → B x) {x y : A}  (p : x ≡ y) →  (transp B p (f x)) ≡ (f y)
...
ℕ-eq-iff-Eqℕ m n  = pair (ind≡ (λ  {x y} →  λ p → Eqℕ x y ) λ x → refl-Eqℕ x) ((indℕ  (λ x →  (Eqℕ x n → (x ≡ n))) (indℕ (λ y → (Eqℕ zero y → (zero ≡ y))) (ind𝟙 (const-type-fam (zero ≡ zero)) (refl zero))  (λ (n₁ : ℕ) f (x₁ : Eqℕ zero (succ n₁)) → ind𝟘 (const-type-fam (zero ≡ succ n₁)) x₁) n) (λ (n₁ : ℕ) f (x₁ : Eqℕ (succ n₁) n)  → {!!}) m)  )
